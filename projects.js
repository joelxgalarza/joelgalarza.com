...
*/
function clearAll() {
  let memeParagraph = document.getElementById('text-above-meme');
  let jokeParagraph = document.getElementById('text-above-joke');
  let quoteParagraph = document.getElementById('text-above-quote');
  let riddleParagraph = document.getElementById('text-above-riddle');

  if (memeParagraph.querySelector('img')) {
    memeParagraph.querySelector('img').remove(); // Remove the existing meme
  }
  
  if (jokeParagraph.querySelector('p')) {
    jokeParagraph.querySelector('p').remove(); // Remove the existing joke
  }

  if (quoteParagraph.querySelector('p')) {
    quoteParagraph.querySelector('p').remove(); // Remove the existing quote
  }

  if (riddleParagraph.querySelector('p')) {
    riddleParagraph.querySelector('p').remove(); // Remove the existing riddle question
  }

  if (riddleParagraph.querySelector('#riddle-answer')) {
    riddleParagraph.querySelector('#riddle-answer').remove(); // Remove the existing riddle answer
  }
}
...
function showJoke() {
  ...
  let textNode = document.createTextNode(randomJokeText);
  element.appendChild(textNode);

  jokeParagraph.appendChild(element);
}
...
function showQuote() {
  ...
  let element = document.createElement('p');
  let textNode = document.createTextNode(randomQuote.quote + " - " + randomQuote.author);
  let quoteParagraph = document.getElementById('text-above-quote');

  element.appendChild(textNode);
  quoteParagraph.append(element);
}
...
function showRiddle() {
  ...
  const riddleParagraph = document.getElementById('text-above-riddle');

  const questionElement = document.createElement('p');
  questionElement.textContent = question;
  riddleParagraph.appendChild(questionElement);

  const answerElement = document.createElement('p');
  answerElement.textContent = answer;
  answerElement.setAttribute('id', 'riddle-answer');
  answerElement.hidden = true;
  riddleParagraph.appendChild(answerElement);
}
...
function revealAnswers() {
  ...
  if (!riddleParagraph.contains(answerElement)) {
    alert("There is no riddle to reveal the answer to!");
  } else if (answerElement.hidden) {
